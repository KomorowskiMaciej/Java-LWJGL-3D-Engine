...
public class ShadowBox {
    ...
    private CameraBaseComponent cam;

    private float farHeight, farWidth, nearHeight, nearWidth;

    protected ShadowBox(Matrix4f lightViewMatrix, CameraBaseComponent camera) {
    ...
    protected void update() {
        ...
        Vector3f forwardVector = new Vector3f(Matrix4f.transform(rotation, Config.getShadowForward(), null));

        Vector3f toFar = new Vector3f(forwardVector);
        toFar.scale(Config.getShadowDistance());
        Vector3f toNear = new Vector3f(forwardVector);
        toNear.scale(Config.getNearPlane());
        Vector3f centerNear = Vector3f.add(toNear, cam.getGameObject().getPosition(), null);
        Vector3f centerFar = Vector3f.add(toFar, cam.getGameObject().getPosition(), null);
    ...
    private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
        ...
        Vector3f upVector = new Vector3f(Matrix4f.transform(rotation, Config.getShadowUp(), null));
    ...
    private Matrix4f calculateCameraRotationMatrix() {
        ...
        rotation.rotate((float) Math.toRadians(-cam.getGameObject().getRotation().y), new Vector3f(0, 1, 0));
        rotation.rotate((float) Math.toRadians(-cam.getGameObject().getRotation().x), new Vector3f(1, 0, 0));
        return rotation;
    }


    private void calculateWidthsAndHeights() {
        farWidth = (float) (Config.getShadowDistance() * Math.tan(Math.toRadians(Config.getFOV())));
        nearWidth = (float) (Config.getNearPlane()
                * Math.tan(Math.toRadians(Config.getFOV())));
